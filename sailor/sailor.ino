...
void loop()
{
    ...
    //  radio.write( mode, sizeof(mode) );
      ...
      int dirServoDegree = map(data[0], 0, 1023, 45, 135) - (data[7] - 512) / 25;
      ...
      int motorASpd = data[0] - 512 + (data[6] - 512) / 10;
      bool motorADir = motorASpd > 0 ? FORWARD: BACKWARD;
      int motorBSpd = data[1] - 512 + (data[6] - 512) / 10;
      bool motorBDir = motorBSpd > 0 ? FORWARD: BACKWARD;
      motorASpd = abs(constrain(motorASpd, - 512, 512));
      motorBSpd = abs(constrain(motorBSpd, - 512, 512));
      motorASpd = map(motorASpd, 0, 512, 0, 255);
      motorBSpd = map(motorBSpd, 0, 512, 0, 255);
      // control the steering and travelling of the smart car
      ctrlCar0(dirServoDegree,ultrasonicServoDegree, motorADir, motorASpd, motorBDir, motorBSpd);
      switch(RGBVal){
...
void ctrlCar0(byte dirServoDegree,byte ultrasonicServoDegree, bool motorADir, byte motorASpd, bool motorBDir, byte motorBSpd) {
//  dirServo.write(dirServoDegree + dirServoOffset);
//  ultrasonicServo.write(ultrasonicServoDegree + ultrasonicServoOffset);
  digitalWrite(dirAPin, motorADir);
  digitalWrite(dirBPin, motorBDir);
  analogWrite(pwmAPin, motorASpd);
  analogWrite(pwmBPin, motorBSpd);
}
